...
typedef struct {
	uint8_t const opcode = CMD_GET_INFO_OPCODE;
} PACKED CmdGetInfo;

#define CMD_CREATE_SCANNER_OPCODE 1
typedef struct {
	uint8_t const opcode = CMD_CREATE_SCANNER_OPCODE;
	uint32_t scan_id;
} PACKED CmdCreateScanner;

#define CMD_REMOVE_SCANNER_OPCODE 2
typedef struct {
	uint8_t const opcode = CMD_REMOVE_SCANNER_OPCODE;
	uint32_t scan_id;
} PACKED CmdRemoveScanner;

#define CMD_CREATE_CONNECTION_CHANNEL_OPCODE 3
typedef struct {
	uint8_t const opcode = CMD_CREATE_CONNECTION_CHANNEL_OPCODE;
	uint32_t conn_id;
	uint8_t bd_addr[6];
...
typedef struct {
	uint8_t const opcode = CMD_REMOVE_CONNECTION_CHANNEL_OPCODE;
	uint32_t conn_id;
} PACKED CmdRemoveConnectionChannel;

#define CMD_FORCE_DISCONNECT_OPCODE 5
typedef struct {
	uint8_t const opcode = CMD_FORCE_DISCONNECT_OPCODE;
	uint8_t bd_addr[6];
} PACKED CmdForceDisconnect;

#define CMD_CHANGE_MODE_PARAMETERS_OPCODE 6
typedef struct {
	uint8_t const opcode = CMD_CHANGE_MODE_PARAMETERS_OPCODE;
	uint32_t conn_id;
	enum LatencyMode latency_mode;
	int16_t auto_disconnect_time;
...
typedef struct {
	uint8_t const opcode = CMD_REMOVE_BATTERY_STATUS_LISTENER_OPCODE;
	uint32_t listener_id;
